...
def make_mnist_vec_fc(mono=False):
    model = nn.Sequential((vnn.Linear(10, 28*28*10, 1024, first_layer=True, mono=mono),
                           vnn.tReLU(),
...
def make_mnist_nonvec_fc():
    model = nn.Sequential((nn.Linear(28*28, 1024),
                           nn.ReLU(),
                           nn.Linear(1024, 512),
                           nn.ReLU(),
                           nn.Linear(512, 10)))
...
def make_cifar_vec_fc(mono=False):
    model = nn.Sequential((vnn.Linear(10, 32*32*3*10, 1024, first_layer=True, mono=mono),
                           vnn.tReLU(),
                           vnn.Linear(10, 1024, 512, mono=mono),
                           vnn.tReLU(),
...
def make_mnist_vec_conv(mono=False):
    model = nn.Sequential((vnn.Conv2d(10, 10, 128, 3, stride=1, padding=1, first_layer=True, mono=mono),
                           vnn.ctReLU(),
                           vnn.AvgPool2d(2),
...
def make_mnist_nonvec_conv():
    model = nn.Sequential((nn.Conv2d(1, 128, 3, stride=1, padding=1),
                           nn.ReLU(),
                           nn.AvgPool2d(2),
...
def make_cifar_vec_conv(mono=False):
    model = nn.Sequential((vnn.Conv2d(10, 30, 256, 5, stride=1, padding=2, first_layer=True, mono=mono),
                           vnn.ctReLU(),
                           vnn.AvgPool2d(2),
...
def make_cifar_nonvec_conv(mono=False):
    model = nn.Sequential((nn.Conv2d(3, 256, 5, stride=1, padding=2),
                           nn.ReLU(),
                           nn.AvgPool2d(2),
...
def make_mnist_vec_lc(mono=False):
    model = nn.Sequential((vnn.VecLocal2d(10, 10, 128, 3, h_in=28, w_in=28, stride=1, padding=1, first_layer=True, mono=mono),
                           vnn.ctReLU(),
                           vnn.AvgPool2d(2),
                           ...
                           vnn.AvgPool2d(2), #7 by 7
                           vnn.Flatten(),
                           vnn.Linear(10, 6272, 1024),
                           vnn.tReLU(),
                           vnn.Linear(10, 1024, 1)))
    return model

def make_mnist_nonvec_lc():
    model = nn.Sequential((vnn.VecLocal2d(1, 128, 3, h_in=28, w_in=28, stride=1, padding=1),
                           nn.ReLU(),
                           nn.AvgPool2d(2),
...
def make_cifar_vec_lc(mono=False):
    model = nn.Sequential((vnn.VecLocal2d(10, 30, 128, 5, h_in=32, w_in=32, stride=1, padding=2, first_layer=True, mono=mono),
                           vnn.ctReLU(),
                           vnn.AvgPool2d(2),
                           ...
                           vnn.ctReLU(),
                           vnn.Flatten(),
                           vnn.Linear(10, 2048, 1024),
                           vnn.tReLU(),
                           vnn.Linear(10, 1024, 1)))
    return model

def make_cifar_nonvec_lc():
    model = nn.Sequential((vnn.VecLocal2d(3, 128, 5, h_in=32, w_in=32, stride=1, padding=2),
                           nn.ReLU(),
                           nn.AvgPool2d(2),
