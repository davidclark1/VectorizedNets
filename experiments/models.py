...
def make_cifar_vec_lc(mono=False):
    model = nn.Sequential(
        vnn.Local2d(10, 30, 32, 5, h_in=32, w_in=32, stride=1, padding=2, first_layer=True, mono=mono),
        vnn.ctReLU(10, 32, 32, 32),
        vnn.AvgPool2d(2),
        vnn.Local2d(10, 32, 32, 5, h_in=16, w_in=16, stride=1, padding=2, mono=mono),
        vnn.ctReLU(10, 32, 16, 16),
        vnn.AvgPool2d(2),
        vnn.Local2d(10, 32, 32, 2, h_in=8, w_in=8, stride=2, padding=0, mono=mono),
        vnn.ctReLU(10, 32, 4, 4),
        vnn.Flatten(),
        vnn.Linear(10, 512, 1024, mono=mono),
        vnn.tReLU(10, 1024),
        vnn.Linear(10, 1024, 1, mono=mono))
    return model
...
def make_cifar_nonvec_lc():
    model = nn.Sequential(
        local2d.Local2d(3, 32, 5, h_in=32, w_in=32, stride=1, padding=2),
        nn.ReLU(),
        nn.AvgPool2d(2),
        local2d.Local2d(32, 32, 5, h_in=16, w_in=16, stride=1, padding=2),
        nn.ReLU(),
        nn.AvgPool2d(2),
